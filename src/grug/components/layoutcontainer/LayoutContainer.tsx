        ...
        <div ref={ref} className={styles.gridRoot}>
            <div className={styles.scrollingContent} style={{height: snapshot.contentHeight}}>
                {snapshot.childLayoutData.map((layoutData) => (
                    (
                        <div key={layoutData.uniqueId} className={styles.tileWrapper} style={stylesForLayoutData(layoutData, enableTransition)}>
 ...
 */
function stylesForLayoutData(layoutData: ItemLayoutData, enableTransition: boolean): CSSProperties {
    return {
        position: "absolute",
